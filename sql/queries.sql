...
FROM
  ...
  JOIN
  KARNATAKA_2018 GER ON (GER.CONSTITUENCY = WINNER.CONSTITUENCY
...
FROM
  KARNATAKA_2018 GER1
  JOIN
  KARNATAKA_2018 GER2 ON (GER1.PARTY = GER2.PARTY
...
FROM
  ...
  (SELECT
   ...
   FROM
     KARNATAKA_2018) AS TOTAL_VOTE_DETAILS
...
FROM
  ...
  JOIN
  KARNATAKA_2018 ger ON VOTE_DETAILS.CONSTITUENCY = ger.CONSTITUENCY
   ...
   FROM
     (SELECT
      ...
      FROM
        KARNATAKA_2018
      GROUP BY STATE , CONSTITUENCY) AS VOTE_DETAILS
     JOIN KARNATAKA_2018 ger ON (VOTE_DETAILS.CONSTITUENCY = ger.CONSTITUENCY)
...
FROM
  (SELECT
     (COUNT(GER.CANDIDATE) * 1600000) AS TOTAL_COST,
     GER.PARTY AS PARTY,
     PARTY_VOTE_DETAILS.PARTY_VOTES AS PARTY_VOTES
   FROM
     KARNATAKA_2018 GER
           ...
           FROM
             (SELECT
              ...
              FROM
                KARNATAKA_2018
              GROUP BY STATE , CONSTITUENCY) WINNER
             JOIN KARNATAKA_2018 GER ON (GER.CONSTITUENCY = WINNER.CONSTITUENCY
                                                   AND GER.STATE = WINNER.STATE)
           WHERE
             GER.VOTES = WINNER.WINNER_VOTES
...
FROM
  (SELECT
   ...
   FROM
     KARNATAKA_2018 GER1
     JOIN KARNATAKA_2018 GER2 ON (GER1.PARTY = GER2.PARTY
                                            AND GER1.CANDIDATE = GER2.CANDIDATE)
   WHERE
     GER1.CONSTITUENCY <> GER2.CONSTITUENCY
